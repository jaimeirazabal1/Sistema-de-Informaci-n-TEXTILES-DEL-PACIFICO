...
class CollectImpl
{
        ...
        public function getByCode($idCollect)
	{
        ...
        public function getPagosBetweenDate($dateA, $dateB)
	{
                $sql = "SELECT recaucodig, recaucredi, credifactu, crediclien, cliennombr, recaufecha, recauvalor, recautipo
				FROM recaudo, credito, cliente "
                . "WHERE recaucredi = credicodig
				AND crediclien = cliencodig
				AND RECAUFECHA BETWEEN TO_DATE('".$dateA." 00:00:00', 'yyyy/mm/dd hh24:mi:ss') 
				AND TO_DATE('".$dateB." 23:59:59', 'yyyy/mm/dd hh24:mi:ss') 
                ORDER BY recaucodig desc";
            $conex = Conexion::getInstancia();
            $stid = oci_parse($conex, $sql);
            oci_execute($stid);
            $foo = array();
