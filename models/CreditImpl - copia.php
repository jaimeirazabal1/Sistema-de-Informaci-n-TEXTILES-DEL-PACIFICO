...
class CreditImpl
{
	...
	{
            $sql = "SELECT cr.CREDICODIG, cr.CREDICLIEN, cr.CREDIFACTU, cr.CREDIFECHA, cr.CREDICONCE, cr.CREDIVALOR, cr.CREDISALDO, cr.CREDIESTAD, cr.CREDIFECCA, cr.CREDIUSUAR FROM credito cr ORDER BY cr.CREDICODIG ASC";
	...
	{
            $sql = "SELECT cr.CREDICODIG, cr.CREDICLIEN, cr.CREDIFACTU, cr.CREDIFECHA, cr.CREDICONCE, cr.CREDIVALOR, cr.CREDISALDO, cr.CREDIESTAD, cr.CREDIFECCA, cr.CREDIUSUAR FROM credito cr WHERE cr.CREDICODIG = ".$idCredit;
            $conex = Conexion::getInstancia();
            $stid = oci_parse($conex, $sql);
            oci_execute($stid);
            $foo = array();
...
//        }
	...
	{
            $sql = "SELECT cr.CREDICODIG, cr.CREDICLIEN, cr.CREDIFACTU, cr.CREDIFECHA, cr.CREDICONCE, cr.CREDIVALOR, cr.CREDISALDO, cr.CREDIESTAD, cr.CREDIFECCA, cr.CREDIUSUAR, cr.CREDIFACRE FROM credito cr WHERE cr.CREDICLIEN = ".$idc." AND cr.CREDIESTAD = 'AC' ORDER BY cr.CREDICODIG ASC";
            $conex = Conexion::getInstancia();
            $stid = oci_parse($conex, $sql);
            oci_execute($stid);
            $foo = array();
        ...
        public function getBill($codeCredit)
	{
        ...
        public function updateStateByID(Credit $objCredit){   
            $sql = "UPDATE credito cr SET cr.CREDIESTAD = '".$objCredit->getState()."' WHERE cr.CREDICODIG = ".$objCredit->getCode();
            $conex = Conexion::getInstancia();
            $stid = oci_parse($conex, $sql);
            oci_execute($stid);            
        } 
...
//            }
            ...
            {
                $sql = "SELECT COUNT(*) FROM credito cr "
                . "WHERE cr.CREDICODIG = ".$credit." AND cr.CREDIFECHA BETWEEN TO_DATE('".$dateA." 00:00:00', 'yyyy/mm/dd hh24:mi:ss') AND TO_DATE('".$dateB." 23:59:59', 'yyyy/mm/dd hh24:mi:ss') ";                
            }
            else if(strcmp($client, "") == 0 && strcmp($credit, "") == 0 && strcmp($bill, "") != 0 ) 
            {
                $sql = "SELECT COUNT(*) FROM credito cr "
                . "WHERE cr.CREDIFACTU = ".$bill." AND cr.CREDIFECHA BETWEEN TO_DATE('".$dateA." 00:00:00', 'yyyy/mm/dd hh24:mi:ss') AND TO_DATE('".$dateB." 23:59:59', 'yyyy/mm/dd hh24:mi:ss') ";
            }            
