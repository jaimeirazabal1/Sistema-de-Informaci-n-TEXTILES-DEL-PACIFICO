...
//Cabecera de página
    function Header() {      
        ...
        $this->Cell(110, 6, 'Ventas de Algodon por Color: ', 0, 0, 'L');
        //$this->Cell(110);
        $this->SetFont('Times', 'B', 12);
        $this->Cell(0, 6, 'VENTAS DE ALGODON POR COLOR', 1, 0, 'C');
    ...
    function TablaSimple($header) {        
		...
		foreach ($arrTemp as $valorStock) {
                   /** $quantityAvailable = $objStockImpl->getQuantityAvailableAlgodon($valorStock->getCode(), $valorStock->getColor());
					...
					$this->Cell(12, 5, $valorStock->getQuantity(), 0, 0, 'R');
        ...
        $this->SetFont('Times', 'B', 12);
        $this->Cell(-90, 5, 'TOTAL VENTAS ALGODON - KGS:', 0, 0, 'R');
