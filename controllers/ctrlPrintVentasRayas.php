...
//        $this->Cell(0, 10, 'Page ' . $this->PageNo() . '/{nb}', 0, 0, 'C');
    ...
    function TablaSimple($header) {        
        ...
        foreach ($objStockImpl->getByVentasRayasBetweenDate($_POST['txbFechaInicio'], $_POST['txbFechaFin'], $_POST['txbReferencia'], $_POST['selectColor']) as $valorStock) {            
            
			$totalKGS += $valorStock->getQuantity();
		...
		foreach ($arrTemp as $valorStock) {
            ...
            $this->Cell(100, 5, utf8_decode($valorStock->getQuantity()), 0);
        ...
        $this->SetFont('Times', 'B', 12);
        $this->Cell(-90, 5, 'TOTAL VENTAS RAYAS - KGS:', 0, 0, 'R');
...
$header = array('        Codigo Color', utf8_decode('Nombre Color'), 'Ventas - KGS');
$pdf->AliasNbPages();
