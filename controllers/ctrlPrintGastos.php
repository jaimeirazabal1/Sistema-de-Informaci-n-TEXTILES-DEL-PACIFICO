...
//Cabecera de página
    function Header() {      
        ...
        //$this->Cell(110);
        $this->SetFont('Times', 'B', 12);
        $this->Cell(0, 6, strtoupper('Reporte de Gastos'), 1, 0, 'C');
    ...
    function TablaSimple($header) {        
        ...
        foreach ($header as $col)
        {
            if($cont == 0)
                ...
                $this->Cell(60, 7, $col, 0, 0, 'C');
            else if($cont == 4)
                $this->Cell(45, 7, $col, 0, 0, 'C');            
        ...
        $data = $client->get_gastos();
        $suma_gasto=0;
        foreach ($data as $value) {            
      
        
            $this->Cell(30, 5, $value['GASTORECIB'], 0, 0, 'L');
            $this->Cell(50, 5, $value['GASTOCLIEN']." ".$value['CLIENNOMBR'], 0, 0, 'R');
            $this->Cell(60, 5, $value['GASTOCONCE']." ".$value['CONCENOMBR'], 0, 0, 'R');
            $this->Cell(60, 5, $value['GASTOFECHA'], 0, 0, 'R');
            $this->Cell(45, 5, number_format($value['GASTOVALOR'],2), 0, 0, 'R');
           
            $this->Ln(5);                
           $suma_gasto+=$value['GASTOVALOR'];
        }  
        ...
        $this->Cell(60, 5, 'TOTAL:', 0, 0, 'R');
        $this->Cell(45, 5, number_format($suma_gasto,2), 0, 0, 'R');
...
$header = array(utf8_decode('CÓDIGO RECIBO'),utf8_decode('CÓDIGO Y NOMBRE CLIENTE'), utf8_decode('CÓDIGO Y NOMBRE DEL CONCEPTO'),utf8_decode('FECHA GENERACIÓN DEL GASTO'),'VALOR DEL GASTO');
$pdf->AliasNbPages();
//Primera página
$pdf->AddPage();
$pdf->SetY(55);
