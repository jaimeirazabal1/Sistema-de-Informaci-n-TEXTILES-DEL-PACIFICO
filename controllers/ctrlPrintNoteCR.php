...
// Cabecera de página
    function Header() {      
        ...
        foreach ($objNoteImpl->getByCode($_GET['id']) as $valorNote) {
            ...
            foreach ($objRemisionImpl->getByCode($valorNote->getCodeBill()) as $valorRemision){            
            ...
            $this->SetFont('Times', 'B', 12);
            $this->SetTextColor(48,131,155);
            $this->Cell(0, 6, 'NOTA CREDITO No. '.$_GET['id'], 1, 0, 'C');
        
            $this->SetFont('Times', '', 9);
            $this->Ln();
            $this->Cell(110, 6, utf8_decode('Identificación: '.$cc), 0, 0, 'L');
            $this->Cell(0, 6, utf8_decode('Remisión No. '.$valorNote->getCodeBill()), 0, 0, 'L');
...
//                foreach ($objDetailRemisionImpl->getByCode($valorNote->getCodeBill()) as $valorDetailRemision) {            
//
//                    $this->Cell(20, 5, utf8_decode($valorDetailRemision->getCodeArticle()), 0);
//
...
//                    $this->Cell(21, 5, $valorDetailRemision->getDevolucion(), 0, 0, 'R');
//                    $this->Cell(21, 5, number_format($valorDetailRemision->getValueUnit() * $valorDetailRemision->getDevolucion()), 0, 0, 'R');                
//                    $this->Ln(5);                
//                }
            }
