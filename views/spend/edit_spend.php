        ...
        <section class="contenedor">
            ...
            {?>
                <section class="contenido" id="contenidoGeneral">
                    <h1>Editar Gasto</h1>
                    <form method="post" action="../../controllers/ctrlEditSpend.php">
                        <?php
                        include_once '../../models/Spend.php';
                        include_once '../../models/SpendImpl.php';
                        $objSpendImpl = new SpendImpl();
                        
                        foreach ($objSpendImpl->getByCode($_GET[id]) as $valor) {   
                            echo '<input type="hidden" value="'.$valor->getCode().'" name="txbCode">';
                            echo '<input type="number" value="'.$valor->getCodeClient().'" name="txbCodeClient" required>';
                            
                            echo '<select name = "selectConcept" id = "selectConcept">';                                
                                include_once '../../models/Concept.php';
                                include_once '../../models/ConceptImpl.php';
                                $objConcepImpl = new ConceptImpl();

                                foreach ($objConcepImpl->getAll() as $valorConcept) {
                                    if($valor->getCodeConcept() == $valorConcept->getCode())
                                        echo '<option value="' . $valorConcept->getCode() . '" selected="selected">' . $valorConcept->getName() . '</option>';
                                    else
                                        echo '<option value="' . $valorConcept->getCode() . '">' . $valorConcept->getName() . '</option>';
                                }
                            echo '</select>'; 
                            
                            
                            
                            echo '<input type="text" id="example2" value="'.$valor->getValue().'" name="txbValue" placeholder="Código" maxlength="12" required>';
