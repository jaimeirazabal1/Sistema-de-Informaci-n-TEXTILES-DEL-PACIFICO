    ...
    <body>
        <section class="contenedor">
            ...
            <section class="contenido" id="contenidoGeneral2">
                <h1>Ventas Rayas X Color</h1>
                
                <?php
                if($_POST)
                {?>
                    <form id="formVentasRayas" method="post">
                        <?php 
                        ...
                        ?>                        
                        <input id="btnVentasRayas" type="submit" value="Consultar">
                        <input id="btnGenerarVentasRayas" type="submit" value="Generar PDF">
                    </form>    
                <?php
                }
                else
                ...
                <div class="listado">                                      
                    ...
                    if($_POST)
                    {
                        ...
                        if($dateFin>=$dateInicio){
                            ...
                            if ($objStockImpl->getCountRayasBetweenDate($_POST['txbFechaInicio'], $_POST['txbFechaFin'], $_POST['txbReferencia']) > 0)
                            {
                            ?>
                                <form method="post">
                                <table>
                                ...
                                foreach ($objStockImpl->getByRayasBetweenDate($_POST['txbFechaInicio'], $_POST['txbFechaFin'], $_POST['txbReferencia'], $_POST['selectColor']) as $valorStock) {
                                    //$quantitySale = $objStockImpl->getSalesAlgodon($valorStock->getCode(), $_POST['txbFechaInicio'], $_POST['txbFechaFin']);
//                                    $quantitySale = $objStockImpl->getQuantitySale($valorStock->getCode(), $valorStock->getColor());
//                                    $quantityAvailable = $quantityAvailable - $quantitySale;
                                    
                                    echo '<tr>
                                        <td>'.$valorStock->getCode().'</td>
                                        <td>'.$valorStock->getName().'</td>
                                        <td>'.$valorStock->getDetalle().'</td>
										<td class="tdDerecha">'.$valorStock->getQuantity().'</td></tr>'; 
