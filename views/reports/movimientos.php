    ...
    <body>
        <section class="contenedor">
            ...
            <section class="contenido" id="contenidoGeneral2">
                <h1>Movimientos por Artículo</h1>
                
                <?php
                if($_POST)
                {?>
                    <form id="formReporteMovimArticle" method="post">
                        <?php 
                        ...
                        ?>                        
                        <input id="btnConsultarMovimArticulo" type="submit" value="Consultar">
                        <input id="btnGenerarMovimArticulo" type="submit" value="Generar PDF">
                    </form>    
                <?php
                }
                else
                ...
                <div class="listado">                                      
                    ...
                    if($_POST)
                    {
                        ...
                        foreach ($objStockImpl->getByArticleMovimBetweenDate($_POST['txbFechaInicio'], $_POST['txbFechaFin'], $_POST['txbReferencia']) as $valorStock) {
                            echo '<tr>
                                <td>'.$valorStock->getRemision().'</td>
                                <td>'.$valorStock->getCliente().'</td>
                                <td>'.$valorStock->getName().'</td>';
                            echo '<td class="tdDerecha">'.($valorStock->getQuantity()).'</td>
                                <td class="tdDerecha">'.number_format($valorStock->getValorUnit(),0).'</td>
                                <td class="tdDerecha">'.number_format($valorStock->getValorTotal(),0).'</td>
                            </tr>';
                        }                                
                        ?>
...
//                        $existencias = $entradas-$salidas;
                        echo '<p>Total Kilos: '.($kilos);                              
                        echo '<p>Valor Total: '.number_format($total);
